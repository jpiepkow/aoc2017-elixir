...
5278	113	4427	569	5167	175	192	3903	155	1051	4121	5140	2328	203	5653	3233	
	...
	def part1() do
		...
		# comment either of the 2 below for part 1 or 2
		devReduce |>
		# Enum.reduce([],&([Enum.max(&1) - Enum.min(&1)] ++ &2)) |>
		Enum.reduce(0,&(&1+&2))
		|>IO.inspect

	end
	def devReduce(arr) do
		arr |> Enum.map(fn(x) -> 
			innerReduce(x) |> IO.inspect
		end)
	end
	def innerReduce(arr) do
		arr |> Enum.reduce_while(0,fn (x,acc) -> 
			val = arr |> Enum.reduce_while(0,fn(y,acc2) ->
				cond do
					rem(x,y)  === 0 && x !== y -> {:halt,[(x/y),true]}
					true -> {:cont, [0,false]}
				end
			end)
			cond do
				Enum.at(val,1) === true -> {:halt,Enum.at(val,0)}
				true -> {:cont,0}
			end
		end)
	end
