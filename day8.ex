...
nm dec 907 if qen == 2233
	...
	def part1() do
		...
		map = commands |> createMap |> to_keyword_list
		cmds = commands |> getRunCommands
		ifs = commands |> getifs
		newMap = runCommands(ifs,cmds,map,0) |> IO.inspect
	end

	def getRunCommands(arr) do
		arr |> Enum.map(fn(x) -> 

			split = String.split(x," ", trim: true)
			mult = if(Enum.at(split,1) == "inc") do
			"+"	
			else
			"-"	
			end
			"#{Enum.at(split,0)} = #{Enum.at(split,0)} #{mult} #{Enum.at(split,2)}"
		end)
	end
	def getifs(arr) do
		arr |> Enum.map(fn(x) -> 
			split = String.split(x," ", trim: true)
			"#{Enum.at(split,4)} #{Enum.at(split,5)} #{Enum.at(split,6)}"
		end)
	end

	def createMap(arr) do
		arr |> getKeys |> Enum.reduce(%{},fn(x,acc) ->
			Map.put(acc,x,0)
		end)
	end
	def runCommands([h|t],[h2|t2],keyword,high) do
		{result,newKeyMap} = Code.eval_string(h,keyword)
		{result2,newKeyMap2} = Code.eval_string(h2,keyword)
		if(result) do
			if(result2 > high) do
				runCommands(t,t2,newKeyMap2,result2)	
			else

				runCommands(t,t2,newKeyMap2,high)	
			end
		else
			runCommands(t,t2,keyword,high)	
		end
	end
	def runCommands([],[],keyword,high) do
		{keyword,high}
	end
	...
	def getKeys(arr) do
		vals =arr |> Enum.map(&(String.split(&1," ")))
		Enum.map(vals,&(Enum.at(&1,0)))
	end

	def getCommands(str) do
		str
		|> String.split( "\n", trim: true) 
	end
